...
public class EditCompanyAction {
    ...
    public ResponseEntity<HttpStatus> doActionOnCompany(CompanyRepository companyRepository,
        ...
        LoggerHelper.logData(Level.INFO, "Edit company", correlationToken, authorization,
                EditCompanyAction.class.getName());
        try {
            CompanyDto companyDtoTemp = mongoTemplate.findOne(
                    new Query(Criteria.where("companyId").is(companyDto.getCompanyId())
                            .andOperator(Criteria.where("customerNumber").is(companyDto.getCustomerNumber()))),
                    CompanyDto.class);
            ...
            LoggerHelper.logData(Level.INFO, "Company successful stored.", correlationToken, authorization,
                    EditCompanyAction.class.getName());
            return new ResponseEntity<HttpStatus>(HttpStatus.OK);
        } catch (Exception e) {
            LoggerHelper.logData(Level.ERROR, "Company unsuccessful stored", correlationToken, authorization,
                    EditCompanyAction.class.getName(), e);
